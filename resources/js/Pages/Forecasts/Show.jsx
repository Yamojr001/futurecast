...
export default function Show({ forecast, auth, userStakeLevel = 0 }) {
  ...
  if (!forecast) {
    return (
      <div className="min-h-screen w-full bg-gradient-to-br from-[#1E203B] to-slate-900 text-white font-sans p-4 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-slate-400 mt-2">The forecast you are looking for does not exist.</p>
        <Link
  ...
  const handleStake = async (amount) => {
    ...
    const onChainForecastId = Number(forecast.id); // <--- Only using `forecast.id`
    setIsLoading(true);
    ...
    try {
      ...
      try {
        ...
        if (!onchain || onchain[0] === 0n) {
          alert("Forecast ID does not exist on-chain. Aborting.");
          setIsLoading(false);
          setFeedback("");
          return;
        }
      } catch (readErr) {
        alert("Forecast ID not found on-chain.");
        setIsLoading(false);
        setFeedback("");
        return;
      }
      ...
      try {
        setFeedback("Confirm in wallet (native)...");
        ...
        console.warn("Native payment failed, falling back to ERC20...", nativeErr);
      }
